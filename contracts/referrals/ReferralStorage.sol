    ...
    IReferralStorage
{
    ...
    function setTier(
        ...
        require(
            _WLPRate <= BASIS_POINTS_DIVISOR,
            "ReferralStorage: invalid WLP Rate"
        );
    ...
    function updateTier(
        ...
        require(
            _vWINRRate <= BASIS_POINTS_DIVISOR,
            "ReferralStorage: invalid vWINR Rate"
        );
    ...
    function setPlayerReferralCodeByUser(bytes32 _code) external {
        require(
            playerReferralCodes[msg.sender] == bytes32(0),
            "ReferralStorage: already set"
        );
    ...
    function _setPlayerReferralCode(address _account, bytes32 _code) private {
        ...
        require(
            ...
            "ReferralStorage: can not set own code"
        );
        // Ensure that the code exists.
        require(
            codeOwners[_code] != address(0),
            "ReferralStorage: code does not exist"
        );
    ...
    function setWithdrawInterval(
        ...
        require(
            ...
            "ReferralStorage: invalid interval"
        );
    ...
    function addReferrerToBlacklist(
        address _referrer,
        bool _setting
    ...
    function _referrerOnBlacklist(
        address _referrer
    ) internal view returns (bool onBlacklist_) {
     ...
     */
    function isPlayerReferred(
        address _player
    ) public view returns (bool isReferred_) {
    ...
    function removeReward(
        ...
        if (referrer_ != address(0)) {
            ...
            if (rewards[referrer_][_token] >= _amountRebate) {
                ...
                // remove the rebate to the referral reserves of the vault
                IVault(vault).removeAsideReferral(
                    _token,
                    rewards[referrer_][_token]
                );
    ...
    function getPendingWLPRewards(
        ...
        for (uint256 i = 0; i < wlTokens_.length; ++i) {
            ...
            uint256 amount_ = rewards[_referrer][token_] -
                withdrawn[_referrer][token_];
    ...
    function getReferrerTier(
        address _referrer
    ) public view returns (Tier memory tier_) {
